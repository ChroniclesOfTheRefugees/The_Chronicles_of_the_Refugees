...
function random_numbers($digits) {
	...
	if(isset($_POST['approve'])){
		...
		$xx2=$_POST['xx1'];
		$sqlx = "UPDATE career SET  a17='$a17',a18='$a18',a19='$a19' WHERE id='$xx2' ";
		...
		try{
			...
			if($update_Result)
			{
				...
				{		 
					phpAlert("Promotions Successful");					  
				}else{}
			}
			} catch (Exception $ex) {
		}
	...
	if(isset($_POST['denied'])){
		$xx='1';
		$xx2=$_POST['xx2'];
		$sqlx = "UPDATE career SET   b13='$xx'  WHERE id='$xx2' ";
...
// Row count from table Total Applicant
    ...
    $qry="select * from career where b5=1 AND b6=0 AND b7=1 AND b8=0 AND b9=0 AND b10=0 AND b13=0  ";
	$resultTotal=mysqli_query($conn,$qry);
	$c1=mysqli_num_rows($resultTotal);  //Total
 
    $qry="select * from career where b5=1 AND b6=0 AND b7=1 AND b8=0 AND b9=0 AND b10=0 AND b13=0  AND a4='Male'";
	$resultTotal=mysqli_query($conn,$qry);
	$c2=mysqli_num_rows($resultTotal); //from leave j
 
    $qry="select * from career where b5=1 AND b6=0 AND b7=1 AND b8=0 AND b9=0 AND b10=0 AND b13=0  AND a4='Female'";
	$resultTotal=mysqli_query($conn,$qry);
	$c3=mysqli_num_rows($resultTotal);  //from 

	
    $qry="select * from career where  b5=0 AND b6=0  AND b7=0  AND b8=0 AND b9=1 AND b10=0 ";
	$resultTotal=mysqli_query($conn,$qry);
	$c4=mysqli_num_rows($resultTotal);  //from 
	
	if($c1===0){$c2=0;}else{$c2=$c2*100/$c1;}
    if($c2===0){$c3=0;}else{$c3=$c3*100/$c1;}
				...
				<section class="invoice wow fade wow fadeIn animated"style="margin-top:0px;margin-top:10px;    border: 2px solid #01ade1;">
					<div class="row">
						<div class="col-xs-12">
							...
							<img class="img-responsive" src="../../img/logo.png" alt="Chania" width="120"  >  <aa style="color:gray;"><i class="fa fa-globe"></i> Selected for Viba</aa>	</center> 
							<small class="pull-centered"><i>Circular Code: <?php echo $p23   ; ?></i></small>
							</h2>
						</div>
					</div>
					<div class="row invoice-info">
						...
						<div class="col-sm-5 invoice-col  "style=" margin-right:10px;margin-bottom:10px;">
								<form class="  " method="POST">
							<address class="bs">
							<center style="background-color:lightgray;">Fillup By Office</center>
							
								<b>&nbsp;Employee ID:</b> <input class=""style="border:0;color: red;background-color:#ffffff;" type="text" disabled name="a16" value="<?php   echo $eid  ; ?>">		</br>
								<b>&nbsp;Responsibility:</b> 
					<select name="a17"style=" color: green;border:1px dotted gray;">          
                            <option value="Graphics Designer"<?php if($p17=="Graphics Designer") echo "selected";?>>Graphics Designer</option>                        
                            <option value="Web Developer"<?php if($p17=="Web Developer") echo "selected";?>>Web Developer</option> 
					    	<option value="Android Developer"<?php if($p17=="Android Developer") echo "selected";?>>Android Developer</option>
					    	<option value="Android Developer"<?php if($p17=="Android Developer") echo "selected";?>>Android Developer</option>                        
                            <option value="Game Developer"<?php if($p17=="Game Developer") echo "selected";?>>Game Developer</option>
					    	<option value="System Designer"<?php if($p17=="System Designer") echo "selected";?>>System Designer</option>
					    	<option value="English Specialist"<?php if($p17=="English Specialist") echo "selected";?>>English Specialist</option>           
					    	<option value="Customer Support"<?php if($p17=="Customer Support") echo "selected";?>>Customer Support</option>           
					    	<option value="Marketing Officer"<?php if($p17=="Marketing Officer") echo "selected";?>>Marketing Officer</option>           
                        </select>
								
								</br>
								<b >&nbsp;Position:</b> 
					<select style="padding-top:3px;margin-top: 3px;margin-bottom: 3px;color: green;border:1px dotted gray;" name="a18">                  
                            <option value="Junior Officer"<?php if($p18=="Junior Officer") echo "selected";?>>Junior Officer</option>                        
                            <option value="Senior Officer"<?php if($p18=="Senior Officer") echo "selected";?>>Senior Officer</option> 
					    	<option value="Executive"<?php if($p18=="Executive") echo "selected";?>>Executive</option>
					    	<option value="Manager"<?php if($p18=="Manager") echo "selected";?>>Manager</option>                        
                        </select>
								</br>
								 
							<b>&nbsp;Salary: </b><input class=""style="border:1px dotted gray;"autofocus type="text" name="a19" value=" <?php   echo $p19  ; ?>">		</br>
							<b>&nbsp;Join Date:</b><input type="date" name="a20" disabled  class="form-control pull-right" value="<?php echo date('Y-m-d'); ?>" />
</br>
							<b>&nbsp;Leave Date:</b>  <input type="date" name="a21"   class="form-control pull-right" value="<?php echo date('Y-m-d'); ?>" /></br>
							<b>&nbsp;Approved By:</b> Admin
							</address>
							
							<input class="invisible" type="text" name="xx1" value="<?php echo $id  ; ?>">		
							<input type="submit" name="approve" value="Promotions" class="btn bg-olive btn-flat">
							</form>
						</div>
					</div>
					<div class="row">
						...
						<div class="col-xs-12">
							...
							<div class="col-xs-6">
							 <div class="col-xs-4">
								...
								<a href="uploads/cv/<?php echo $cv;?>" target="_blank"><input type="submit" value="View CV" class="btn bg-purple btn-flat"></a>	 
								</div>
						 	
								<div class="col-xs-3">
									<form class="row no-print " method="POST">
										<input class="invisible" type="text" name="xx2" value="<?php echo $id  ; ?>">		
										<input type="submit"style="width:100%;" name="denied" value="Leave Job" class="btn bg-maroon btn-flat">
									</form>	
								</div>
								<div class="col-xs-2">
									<form class="row no-print " method="POST">
  ...
  <!--       <button  class="small-box-footer"data-toggle="modal" data-target="#modal-primary" style="border:0px;width:100%;">View More <i class="fa fa-arrow-circle-right"></i></button>-->
        ...
        <div class="col-lg-3 col-xs-6">
          <div class="small-box bg-green">
            <div class="inner">
              <h3><?php echo((int)$c2); ?><sup style="font-size: 20px">%</sup></h3>
              <p>Male Employee</p>
            </div>
            <div class="icon">
        ...
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-yellow">
            <div class="inner">
              <h3><?php echo((int)$c3); ?><sup style="font-size: 20px">%</sup></h3>
              <p>Female Employee</p>
            </div>
            <div class="icon">
              <i class="ion ion-person-add"></i>
								...
								<div class="box-body">
									<table id="example1" class="table table-bordered table-striped">
										...
										<tbody>
											...
											<td>'.'<div class="row no-print">			 <form  class="row no-print " method="POST"> <input class="invisible"type="text" name="v1" value="'.$row[$i]['id'].'">		 <input type="submit" name="view"value="Action" class="btn btn-warning btn-xs"></input> </form>     </div>'.'</td>
											</tr>';	
											$sl=$sl+1;
											}
